...
export default options => {
    ...
    const _renderTimeout = options.renderTimeout || 300;
    ...
    const filterNodes = (nodes, rule, replace) => {
        return nodes.filter(e => e.matches(rule.selectorText.replace(replace, '$1'))).length > 0;
    };
    ...
    const outCss = Array.prototype.map
                        ...
                        .call(rules, rule => {
                            ...
                            try {
                                if (rule instanceof CSSConditionRule) {
                                    ...
                                    const css = Array.prototype.filter
                                        .call(subRules, rule => {
                                ...
                                if (dropKeyframes && (rule instanceof CSSKeyframeRule || rule instanceof CSSKeyframesRule)) {
        ...
        .filter(cssEntry => cssEntry && cssEntry.rules.length > 0)
        .map(cssEntry => cssEntry.rules.join(''))
        .reduce((css, out) => out + css, '');

    return outCss.replace(/\n/g, '').replace(/content: "(.)"/g, (_a, e) => `content: "\\${encodeURI(e).substr(2)}"`);
};
