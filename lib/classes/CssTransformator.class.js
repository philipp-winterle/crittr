...
class CssTransformator {
    constructor(options) {
        ...
        this.CRITICAL_TYPES_TO_KEEP = ["media", "rule", "charset", "font-face"];

        // detect these selectors regardless of whether one or two semicolons are used
        const pseudoSelectorsToKeepRegex = pseudoSelectorsToKeep
            .map((s) => {
                return ":?" + s;
            })
            .join("|");
        ...
        // Not sure the next lines are correct
        this._PSUEDO_SELECTOR_REGEXP = new RegExp(
            pseudoSelectorsToKeepRegex,
            "g"
        );
    }
        ...
        const getCriticalRuleSelectors = (rule, media, selectorMap) => {
            ...
            if (selectorMap.has(ruleKey)) {
                ...
                return rule.selectors.filter((selector) =>
                    critObj.selectors.includes(selector)
                );
            }
            ...
            if (rule.type === "media") {
                if (rule.rules) {
                    rule.rules = rule.rules
                        .map((internalRule) => {
                            const internalRuleKey = Rule.generateRuleKey(
                                internalRule,
                                rule.media
                            );
                            // Get the critical selectors of this media internal rule
                            internalRule.selectors = getCriticalRuleSelectors(
                                internalRule,
                                rule.media,
                                selectorMap
                            );
                            // Create Map entry for exclude of remaining ast
                            criticalSelectorsMap.set(
                                internalRuleKey,
                                internalRule.selectors
                            );
                            return internalRule;
                        })
                        .filter(
                            (internalRule) =>
                                internalRule.selectors !== undefined &&
                                internalRule.selectors.length > 0
                        );
                ...
                rule.selectors = getCriticalRuleSelectors(
                    rule,
                    "",
                    selectorMap
                );
        ...
        let restRules = _astRestRoot.rules.map((rule) => {
            ...
            if (rule.type === "media") {
                if (rule.rules) {
                    rule.rules = rule.rules
                        .map((internalRule) => {
                            const ruleKey = Rule.generateRuleKey(
                                internalRule,
                                media
                            );
                            if (
                                ruleKey !== false &&
                                criticalSelectorsMap.has(ruleKey)
                            ) {
                                const criticalSelectorsOfRule =
                                    criticalSelectorsMap.get(ruleKey);
                                internalRule.selectors =
                                    internalRule.selectors.filter(
                                        (selector) =>
                                            !criticalSelectorsOfRule.includes(
                                                selector
                                            )
                                    );
                            }
                ...
                if (criticalSelectorsMap.has(ruleKey)) {
                    ...
                    rule.selectors = rule.selectors.filter(
                        (selector) =>
                            !criticalSelectorsOfRule.includes(selector)
                    );
                }
